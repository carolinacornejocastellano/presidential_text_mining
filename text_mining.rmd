...
```{r alternative}
# Sentiment analysis
sentiments_spanish <- read.csv("path/to/spanish_sentiment_lexicon.csv",
  header = TRUE
)

sentiment_scores <- df_tokenized %>%
  inner_join(sentiments_spanish, 
  by = c("word" = "spanish_word")) %>%
  group_by(president) %>%
  summarize(sentiment_score = sum(sentiment_value))

# Visualize sentiment scores
ggplot(sentiment_scores, aes(x = president, y = sentiment_score, fill = president)) +
  geom_col() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "President", y = "Sentiment Score", title = "Sentiment Analysis of Presidential Speeches")
```
