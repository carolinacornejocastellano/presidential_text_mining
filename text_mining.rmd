...
In the table below you can see the probability of fitting in one of the topics, together with the original speech.
...
```{r identify each president's top topic from 5 with LDA model}
...
president_topics_5
```
There are very similar topics. Maybe 5 is a larger number. We will reduce the number of topics to 2, with the hypothesis that this will help to make the themes of each topic more distinct. The exercise will be very similar. We first fit the model:
...
df_lda_2
```
Then, we assign a probability to each word:
...
Now, let's find out the 20 most probable words for each topic. This could help us understand what is each topic about.
...
top_terms_topic_2
```
The following plot reveals which themes are covered in each topic. In a broad sense, although there are some words that are repeated —such as _país_, Perú, _peruanos_, _nacional_, etc.—, it seems that the first topic covers more politics areas and the second one covers more socio-economic ideas. For example, the first is about _gobierno_, _política_, _congreso_, _democracia_, _patria_, and others. The second addresses _salud_, _educación_ and _economía_ issues, talks about _desarrollo_, _empresa_ and quantifies things with _millones_ (millions): it is unclear it it refers to millions of soles (as for proposing projects or programs and their expected cost in soles, the peruvian currency) or millions of peruvians (as the number of beneficiaries for each proposed project). 
...
```
Below we can see which speech (and therefore, which president) could be classified in each cluster. The speeches of Alejandro Toledo, Dina Boluarte, Francisco Sagasti, Martín Vizcarra, Ollanta Humala and Valentín Paniagua are more likely to be classified in the first cluster. The four presidents in our sample who took office by constitutional succession and not by popular vote are included in this cluster. That is, four of six members in this topic were not elected. As for political ideologies, it seems that these do not weigh much in the classification, or else the presidents proclaim themselves to be of a certain wing, but lack more ideological development in their rhetoric: Boluarte was the vice-president of Castillo, the rural left-wing rural leader, and Humala entered with a leftist discourse, although his mandate was more right-wing. These are accompanied in the same topic with Toledo, Vizcarra, who belong to the right or center-right.


On the other hand, the two speeches of dictator Alberto Fujimori, the one by Pedro Castillo, and the one by PPK are more likely to enter the 2nd topic. It is confusing that, in this case, PPK is not in the same cluster as Vizcarra and Sagasti, since they were part of the same party: PPK resigned and Vizcarra took over. Faced with Vizcarra's dismissal, Sagasti took office 5 days later. However, in terms of the issues addressed, Castillo was recognized for giving more importance to "social" issues and education -because he was a teacher-, and PPK to economic issues. On this last point, his similarity with Fujimori and his idea of the _revolución productiva de los pequeños_ (the productive revolution of the little ones) makes sense.
...
```
So, to conclude, the following table summarizes more easily which topic each one would belong to, as mentioned above.

```{r identify each president's top topic from 2 with LDA model}
...
Also, in terms of the sentiments of the speeches, one of the packages assigned the vast majority of words as neutral in tone. These results did not seem correct. However, an additional package was able to identify more nuances: 1724 tokens were identified as "positive", being this the mode. This was followed by words classified as trust, negative, and fear. The least used were those of disgust and surprise.
